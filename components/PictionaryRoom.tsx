...
export default function PictionaryRoom({ me, roomCode, isHost = false, onExit }: Props) {
  ...
  const drawerPlayer = players.find((player) => player.isDrawer) || { id: "pending-drawer", name: "الرسام", initials: "؟", color: "#7ce0c0", points: 0, coins: 0, isDrawer: true };
  ...
  useEffect(() => {
    ...
    const applySnapshot = (data: {
      ...
      isDrawer?: boolean;
      room?: { status?: string; current_word?: string | null; round_ends_at?: string | null; word_options?: unknown; round_number?: number; max_rounds?: number } | null;
      ...
      if (Array.isArray(data.players)) setPlayers(data.players.map((item) => { const name = item.users?.nickname || "لاعب"; return { id: item.user_id, name, initials: name.trim().charAt(0) || "ل", color: item.isYou ? "#f5b942" : item.isDrawer ? "#7ce0c0" : "#b9a7ff", points: item.score || 0, coins: item.round_coins || 0, isYou: item.isYou, isDrawer: item.isDrawer }; }));
    ...
    const applyEvent = (event: string, payload: Record<string, unknown>) => {
      ...
      if (event === "round_winner") {
        ...
        setRoundWinner({ name: String(payload.winnerName || "لاعب"), points: Number(payload.points) || 0, coins: Number(payload.coins) || 0 });
        setPlayers((items) => items.map((player) => player.id === winnerId ? { ...player, state: "guessed" } : player));
  ...
  const sendGuess = async (event: FormEvent) => {
    ...
    setPlayers((items) => items.map((player) => player.isYou ? { ...player, state: "guessed" } : player));
