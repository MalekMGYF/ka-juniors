...
export default function PictionaryLobby({ me, onStarted }: Props) {
  ...
  return (
    <section className="pictionary-lobby-shell"><div className="pictionary-lobby">
      ...
      <div className="pictionary-lobby-grid"><div className="pictionary-lobby-card pictionary-create-card"><div className="pictionary-create-badge">أسرع طريقة للعب</div><h2>أنشئ روم جديدة</h2><p>هتاخد كود خاص وتبقى صاحب الروم. ابعته لصحابك واستناهم — لحد 6 لاعبين.</p><label className="pictionary-rounds-select"><span>عدد الجولات</span><select value={maxRounds} onChange={(event) => setMaxRounds(Number(event.target.value))}>{Array.from({ length: 8 }, (_, index) => index + 3).map((rounds) => <option value={rounds} key={rounds}>{rounds} جولات</option>)}</select></label><button className="pictionary-lobby-primary pictionary-create-button" onClick={createRoom} disabled={busy}>{busy ? "بنجهز الروم…" : `＋ إنشاء روم ${maxRounds} جولات`}</button></div><div className="pictionary-lobby-card"><h2>ادخل روم موجودة</h2><p>معاك كود من صاحبك؟ اكتبه هنا وادخل معاه فورًا.</p><input className="pictionary-lobby-input" value={joinCode} onChange={(event) => setJoinCode(event.target.value.toUpperCase())} maxLength={7} placeholder="KJ-ABCD" /><button className="pictionary-lobby-secondary" onClick={joinRoom} disabled={busy}>دخول الروم ←</button></div></div>
      {error && <p className="pictionary-lobby-error">{error}</p>}
