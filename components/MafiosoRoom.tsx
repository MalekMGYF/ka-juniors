...
export default function MafiosoRoom({ code, onLeave }: Props) {
  ...
  useEffect(() => {
    ...
    ["player_joined", "player_left", "room_closed", "game_started", "role_acknowledged", "boss_intro_started", "clue_announced", "discussion_started", "vote_ready_changed", "vote_announcement_started", "voting_started", "message_created", "vote_cast", "vote_resolved", "game_finished"].forEach((event) => channel.on("broadcast", { event }, () => void load()));
    channel.subscribe();
  ...
  return <section className="mafioso-room-shell mafioso-v2-shell">
    ...
    {room?.status === "voting" && <section className="mafioso-voting-focus"><div className="mafioso-boss-note vote"><span>التصويت مفتوح</span><b>اختار شخص واحد بس — ومينفعش تصوّت على نفسك.</b>{data.canVote ? data.hasVoted ? "صوتك اتسجل." : "اختار المتهم." : "أنت متفرج في الجولة دي."}</div><div className="mafioso-vote-list">{players.filter((p: any) => p.status === "active" && !p.isYou).map((player: any) => <button disabled={!data.canVote || data.hasVoted || busy} onClick={() => void post("vote", { targetId: player.userId })} key={player.userId}><span>{player.nickname.charAt(0)}</span><b>{player.displayName}</b><small>{player.voteCount || 0} أصوات</small></button>)}</div></section>}

    {isBossScene && <div className="mafioso-scene-overlay boss" role="dialog" aria-modal="true"><section><span className="mafioso-boss-emblem">✦</span><small>مافيا بوص بيقول</small><h2>{data.case?.title}</h2><p>{data.case?.briefing}</p><div className="mafioso-scene-motives">{(data.motives || []).map((motive: any) => <p key={motive.roleName}><b>{motive.roleName}</b><span>{motive.motive}</span></p>)}</div><em>الدليل الأول هينزل بعد {time}</em></section></div>}
