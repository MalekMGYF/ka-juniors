...
export default function MafiosoRoom({ code, onLeave, onRoomChange }: Props) {
  ...
  useEffect(() => {
    ...
    ["player_joined", "player_left", "room_closed", "game_started", "role_acknowledged", "boss_intro_started", "boss_intro_acknowledged", "clue_announced", "discussion_started", "vote_ready_changed", "vote_announcement_started", "voting_started", "message_created", "vote_cast", "vote_resolved", "game_finished", "rematch_ready_changed", "suspicion_changed", "inspection_completed"].forEach((event) => channel.on("broadcast", { event }, () => void load()));
    channel.subscribe();
  ...
  const inviteUrl = typeof window === "undefined" ? `/mafioso?code=${encodeURIComponent(code)}` : `${window.location.origin}/mafioso?code=${encodeURIComponent(code)}`;
  const copyInvite = async () => {
    try {
      await navigator.clipboard.writeText(inviteUrl);
      setInviteCopied(true);
      window.setTimeout(() => setInviteCopied(false), 1800);
    } catch { setError("انسخ كود الروم يدويًا: " + code); }
  };
  ...
  return <section className="mafioso-room-shell mafioso-v2-shell">
    ...
    {room?.status === "discussion" && <><div className="mafioso-boss-note clue"><span>الدليل رقم {room.round_number}</span><b>{data.currentClue}</b></div>{data.currentEvent && <div className="mafioso-event-card"><span>حدث الجولة</span><b>{data.currentEvent}</b><small>خدوا الحدث ده في اعتباركم قبل التصويت.</small></div>}<section className="mafioso-side-grid"><article className="mafioso-motives"><h2>دوافع الشخصيات</h2>{(data.motives || []).map((motive: any) => <p key={motive.roleName}><b>{motive.roleName}</b><span>{motive.motive}</span></p>)}</article><article className="mafioso-chat"><h2>شات القضية</h2><div className="mafioso-messages">{(data.messages || []).slice(-50).map((item: any) => <p key={item.id}><b>{item.author}</b><span>{item.body}</span></p>)}{!data.messages?.length && <small>ابدأوا كلامكم، الدليل قدامكم.</small>}</div><form onSubmit={sendMessage}><input value={message} onChange={(event) => setMessage(event.target.value)} disabled={!data.canChat || busy} maxLength={420} placeholder={data.canChat ? "اكتب اللي لاحظته…" : "الشات مقفول دلوقتي"} /><button disabled={!data.canChat || busy}>إرسال</button></form></article><article className="mafioso-discussion-ready"><div className="mafioso-discussion-ready-head"><div><span>اختصار وقت النقاش</span><h2>جاهزين للتصويت؟</h2><p>لما كل اللاعبين اللي لسه في القضية يأكدوا، التصويت هيفتح فورًا.</p></div><b>{discussionReady.readyCount}/{discussionReady.requiredCount}</b></div><div className="mafioso-ready-roster">{players.filter((player: any) => player.status === "active").map((player: any) => <span className={player.isReadyForVote ? "is-ready" : ""} key={player.userId}>{player.isReadyForVote ? "✓" : "…"} {player.displayName}</span>)}</div><button className="mafioso-ready-button" type="button" onClick={() => void post("ready_for_vote")} disabled={!discussionReady.canRequestVote || discussionReady.isYouReady || busy}>{discussionReady.isYouReady ? "أنت جاهز للتصويت ✓" : discussionReady.canRequestVote ? "جاهز للتصويت" : "أنت متفرج في الجولة دي"}</button></article></section><section className="mafioso-extra-grid"><article className="mafioso-suspicion-panel"><span>مؤشر الشك</span><h2>مين قصته مش راكبة؟</h2><p>اختار مشتبه واحد. اللي يظهر للجميع هو عدد إشارات الشك فقط، من غير أسماء المصوتين.</p><div>{players.filter((player: any) => player.status === "active" && !player.isYou).map((player: any) => <button type="button" className={data.suspicions?.yourTargetId === player.userId ? "selected" : ""} onClick={() => void post("set_suspicion", { targetId: player.userId })} disabled={busy || Boolean(data.suspicions?.isYouSet)} key={player.userId}><b>{player.displayName}</b><small>{data.suspicions?.counts?.[player.userId] || 0} شكوك</small></button>)}</div></article>{data.ownCard?.specialAbility === "investigator" && <article className="mafioso-investigator-panel"><span>قدرة خاصة</span><h2>المحقق</h2>{data.inspection ? <p>تحققت من لاعب: <b>{players.find((player: any) => player.userId === data.inspection.targetId)?.displayName || "لاعب"}</b> — النتيجة: <strong>{data.inspection.alignment === "mafia" ? "مافيوسو" : "بريء"}</strong></p> : <><p>مرة واحدة في القضية، اختار لاعب تعرف انتماءه سرًا.</p><div>{players.filter((player: any) => player.status === "active" && !player.isYou).map((player: any) => <button type="button" onClick={() => void post("inspect_player", { targetId: player.userId })} disabled={busy} key={player.userId}>افحص {player.displayName}</button>)}</div></>}</article>}</section></>}
